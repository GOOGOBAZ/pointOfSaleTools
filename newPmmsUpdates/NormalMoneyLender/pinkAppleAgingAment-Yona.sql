...
FROM 
    items 
INNER JOIN 
    units_of_measure ON items.unit_id = units_of_measure.unit_id 
INNER JOIN 
    categories ON items.category_id = categories.category_id 
INNER JOIN 
    brands ON items.brand_id = brands.brand_id
 INNER JOIN 
    colors ON items.color_id = colors.color_id
 INNER JOIN 
    sizes ON items.size_id = sizes.size_id
     INNER JOIN 
    forms ON items.form_id = forms.form_id\G
...
SELECT
    ...
    um.unit_name,
    s.stock_level,
    sd.cost_price AS latest_cost_price,
    sd.selling_price AS latest_selling_price,
    s.last_order_date,
      s.last_receipt_date,
        s.last_sale_date
...
JOIN
    ...
    JOIN
    units_of_measure um ON i. unit_id = um. unit_id
JOIN
    (
        SELECT
            sd_inner.stock_id,
            sd_inner.cost_price,
            sd_inner.selling_price,
            sd_inner.stock_details_in,
            sd_inner.stock_details_out,
            sd_inner.expiration_date,
            sd_inner.serial_batch_number
        FROM
            stock_details sd_inner
        WHERE
            sd_inner.stock_details_id IN (
                SELECT MAX(sd2.stock_details_id)
                FROM stock_details sd2
                GROUP BY sd2.stock_id
            )
    ) sd ON s.stock_id = sd.stock_id\G
