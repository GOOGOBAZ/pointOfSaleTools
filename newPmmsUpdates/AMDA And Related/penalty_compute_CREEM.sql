-- SELECT loanTrnId, COUNT(loanTrnId) FROM  amdapenaltycomputenow GROUP BY loanTrnId HAVING COUNT(loanTrnId) > 1;
-- DELETE t1 FROM amdapenaltycomputenow t1 INNER JOIN amdapenaltycomputenow t2 WHERE t1.id < t2.id AND t1.loanTrnId = t2.loanTrnId;
...
IF endDate>=(NOW()-INTERVAL 1 DAY) THEN
 ...
 SELECT COUNT(instalment_no) INTO numberOfItems FROM new_loan_appstoreamort WHERE instalment_due_date<=(NOW()-INTERVAL 3 DAY) AND NOT instalment_status='P' AND master1_id=trnId;



 SELECT instalment_no INTO firstInstalment FROM new_loan_appstoreamort WHERE instalment_due_date<=(NOW()-INTERVAL 3 DAY) AND NOT instalment_status='P' AND master1_id=trnId ORDER BY instalment_no ASC LIMIT 1;
