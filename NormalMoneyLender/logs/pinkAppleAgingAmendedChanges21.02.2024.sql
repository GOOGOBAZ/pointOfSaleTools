...
CREATE PROCEDURE createdRenewedLoan(
  ...
  IN periodTypeString VARCHAR(60),IN userId INT,IN interestRegime DOUBLE,IN initialDisburseDate DATE,IN loanOfficerId INT,IN thePeriodSet DOUBLE,IN theCompuM INT,IN batchNumber VARCHAR(30),IN buzId INT,IN renewals INT,IN theDisId INT, IN theLoanProductId INT) BEGIN
...
ADD COLUMN loan_product_id INT(11) NULL AFTER TotalAccruedInterestPaid;

ALTER TABLE new_loan_appstore1
ADD COLUMN loan_product_id INT(11) NULL AFTER OtherFive;

ALTER TABLE pmms.loandisburserepaystatement
ADD COLUMN loan_product_id INT(11) NULL AFTER OtherFour;
...
CREATE PROCEDURE createNewLoan(
  IN accountNumber VARCHAR(60),
  IN amount DOUBLE,
  IN rate DOUBLE,
  IN tenure DOUBLE,
  IN currentDate DATE,
  IN periodTypeNumber DOUBLE,
  IN periodTypeString VARCHAR(60),
  IN userId INT,
  IN interestRegime DOUBLE,
  IN amortizationDate DATE,
  IN loanOfficerId INT,
  IN thePeriodSet DOUBLE,
  IN theCompuM INT,
  IN batchNumber VARCHAR(30),
  IN buzId INT,
  IN theLoanProductId INT) BEGIN
DECLARE theLoanTxnId,timeP,timeP1 INT;
DECLARE theLoanId VARCHAR(60);
...
-- SELECT @startDate,@nextDate,@endDate;
 ...
 CALL accountNma(accountNumber,@accountName);
-- SELECT buzId;
INSERT INTO  new_loan_appstore VALUES(
  theLoanTxnId,
  amortizationDate,
  theLoanId,
  tenure,
...
-- 31/01/2024	31/01/2024	MWILI Etablishments Ltd 0778667297s Account Deposit for Loan Payment
  -- Dated 31/01/2024	-	3000000.0	MWILI Etablishments Ltd 0778667297

-- SELECT  id_7, customer_name , customer_contact , date_taken,due_date, loans_remaining , principal_remaining ,interest_remaining, principal_inarrears , interest_inarrears , number_of_days_in_arrears ,loan_deadline FROM aging_loan_analysis7 INTO OUTFILE 'aag.sql' FIELDS TERMINATED BY '#' LINES TERMINATED BY '\n';
...
CREATE TABLE penalty_config (
    ...
    max_penalty_rate DECIMAL(10, 2), -- Minimum penalty rate
...
ADD COLUMN penalty_computation_flag BOOLEAN NOT NULL DEFAULT FALSE AFTER instalment_status,
ADD COLUMN penalty_computation_date DATE DEFAULT NULL AFTER penalty_computation_flag,
ADD COLUMN additional_data DECIMAL(10, 2) DEFAULT NULL AFTER penalty_computation_date;




CREATE TABLE loan_product_settings (
  loan_product_settings_id INT AUTO_INCREMENT PRIMARY KEY,
  product_id INT NOT NULL, -- Link to specific loan products
  is_interest_fixed_amount BOOLEAN NOT NULL DEFAULT FALSE, -- Indicates if the interest is a fixed amount
  period_for_interest_calculation ENUM('WEEK', 'MONTH', 'YEAR') NOT NULL, -- Frequency of interest calculation
  min_interest_rate_value DECIMAL(10, 2),
  max_interest_rate_value DECIMAL(10, 2),
        default_interest_rate_value DECIMAL(10, 2),
 min_tenure INT, -- Minimum tenure for the loan
  max_tenure INT, -- Maximum tenure for the loan
 default_tenure INT, -- Maximum tenure for the loan
  repayment_cycle ENUM('DAILY', 'WEEKLY', 'MONTHLY', 'ANNUALLY') NOT NULL, -- Tenure type of the loan

  interest_regime ENUM('FLAT', 'REDUCING_EQUAL_INSTALMENT', 'REDUCING_REDUCING_INSTALMENT') NOT NULL, -- Flat or Reducing interest

  the_reducing_baltype ENUM('SIMPLE_INTEREST', 'COMPOUND_INTEREST') NOT NULL, -- Tenure type of the loan
 
   is_period_for_amort_set BOOLEAN NOT NULL DEFAULT FALSE, -- Indicates if the penalty is a fixed amount
  the_amort_period INT, -- Maximum tenure for the loan
        min_loan_amount DECIMAL(14, 2), -- Maximum amount that can be loaned
     max_loan_amount DECIMAL(14, 2), -- Maximum amount that can be loaned
...
CREATE TABLE guiControlIds (
  id INT NOT NULL PRIMARY KEY,
    SyIdA11X int(11) DEFAULT 1,
      SyIdA12X int(11) DEFAULT 1,
        SyIdA13X int(11) DEFAULT 1,
          SyIdA14X int(11) DEFAULT 1,
            SyIdA15X int(11) DEFAULT 1,
              SyIdA16X int(11) DEFAULT 2,
                SyIdA17X int(11) DEFAULT 2,
...
ALTER TABLE pmms.guiControlIds modify column  id enum('1') NOT NULL;
...
SELECT contactName1 AS name,backUpContactNotification1 AS contact,TIME(CURRENT_TIMESTAMP) AS theTime,DATE_FORMAT(DATE(CURRENT_TIMESTAMP),"%d/%m/%Y") AS theDate FROM  backupcontact WHERE enabled=2 LIMIT 3;
...
Appendix4: Adjustments made due to overstatements, omissions and under statements
	 LIST OF ADJUSTMENTS 	
Details	 DR 	 CR 	Auditor’s comments
Accumulated Depreciation	        1,390,000 		Being an adjustment to correct overstated depreciation for PPE
Depreciation		           1,390,000 	
Amortization	        2,189,876 		Being an adjustment to recognize amortization charge for Computer software 
Acc depn/ Amortization		           2,189,876 	
Prepaid rent	16,000,000		Being an adjustment to correct the classification of prepaid rent done for Feb 2024 to Dec 2024
Accounts receivable		         16,000,000 	
Rent expense	4,800,000		Being an adjustment to expense rent for the months of Jan 2023 to March 2023
Prepaid rent		           4,800,000 	
Prepaid rent	1,600,000		Being an adjustment to correctly wrongly recognized Prepaid rent
Rent Expense		           1,600,000 	
PAYE expense	1,442,152		Being an adjustment to recognize PAYE Liability for the Month of December 2023 not posted
PAYE payable		           1,442,152 	
NSSF Expense	           868,077 		Being an adjustment to recognize NSSF Contribution for December 2023 not recognized
National Social Security Fund Payable 		              868,077 	
Other incomes	        5,761,791 		Being an adjustment to reclassify interest received from the bank -from Other Incomes to Bank Interest received.
Interest from Bank		           5,761,791 	
Retained Earnings	      47,079,611 		Being an adjustment to recognize statutory appropriations made for the FY ended 31-Dec-2023
Statutory Reserve		         29,424,757 	
Development Fund		         14,712,378 	
Education Fund		           2,942,476

-- CALL postingTxnsX(NULL,'2024-01-31','MWILI Etablishments Ltd 0778667297s Account Deposit for Loan Payment\n  Dated 31/01/2024','2024-01-31','3000000.0','-','7995700.0','01128000110','MWILI Etablishments Ltd 0778667297','000zib','BTN43184','GenXX','10002','14:43:56','138','05502063410','05502000010','Dr','Main','NA');

-- CALL adjustIds(1012,'01107000110');

-- CALL adjustIds(13,'01107000010');

--  UPDATE new_loan_appstore SET   TotalPrincipalRemaining=200000,TotalPrincipalPaid=200000 WHERE    trn_id= 71235;

-- UPDATE new_loan_appstore1 SET   TotalPrincipalRemaining=200000,TotalPrincipalPaid=200000 WHERE    trn_id= 71235;

-- UPDATE new_loan_appstoreamort SET PrincipalPaid= 33334, instalment_paid= 41334    WHERE    trn_id=15738;


CALL postingTxnsX(NULL,,'2024-03-12','Loan Payment from Nkamukunda Juliets   loan repayment.\n  Paid on 12/03/2024','2024-03-12','-','2.0','6.1663265E7','05502040210','Nkamukunda Juliet','asms0001','BTN47188','GenXX','10006','23:39:46','216','01128000110','01128000010','Cr','Main','NA');
