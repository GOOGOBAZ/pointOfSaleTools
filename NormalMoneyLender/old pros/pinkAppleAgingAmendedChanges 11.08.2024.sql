...
-- 76329	SEBYONDYA  FRED 0706746597	BODA BODA	240	611000	193497	76903	0	270400	60 DAYS	23/06/2024	10000	Renewed
-- 76637	UZAMUKUNDA PENNINAH 0700796878	marketing	240	424000	258924	50876	0	309800	30 DAYS	24/07/2024	10000	Renewed
-- 76640	mugume bother 0758829744	BODA BODA	240	154400	46313	8967	0	55280	30 DAYS	24/07/2024	10000	Renewed
...
-- SELECT loanId, @accountNumber,@loansOfficer,@balanceDueN,theRenewalRate,@trnDate,@initialDisDate,@loanTenure,periodSet,@theBuzId,@numberRenewals,@previousId,theRateTypeUsed,thePeriodDecoded(TRIM(SUBSTRING_INDEX(@originalTenure,' ',-1))),TRIM(SUBSTRING_INDEX(@originalTenure,' ',-1));
...
END//
 ...
 DROP PROCEDURE IF EXISTS theLoansForRenewalOne;
DELIMITER //
CREATE PROCEDURE theLoansForRenewalOne(IN p_trn_id INT) READS SQL DATA 
BEGIN
    DECLARE theRenewalRate DOUBLE;
    DECLARE theDeadline, numberOfRenewals, theRateTypeUsed, periodSet, l_done, loanId INT;
    DECLARE theDealineMeeasure, theLoanTenurez VARCHAR(30);
    ...
    -- Set loanId to the passed parameter
    SET loanId = p_trn_id;
    ...
    -- Fetch the loan tenure for the given trn_id
...
DROP PREPARE stmt2;
        ...
        IF NOT ISNULL(@accountNumber) THEN
            IF theRateTypeUsed = 1 THEN
                ...
                VALUES (
                    @accountNumber, @loansOfficer, @balanceDueN, theRenewalRate, 
                    @trnDate, @initialDisDate, @loanTenure, periodSet, 
                    @theBuzId, @numberOfRenewals, @previousId, theRateTypeUsed, 
                    thePeriodDecoded(TRIM(SUBSTRING_INDEX(@originalTenure, ' ', -1))), 
                    TRIM(SUBSTRING_INDEX(@originalTenure, ' ', -1))
                );
            ELSE
                INSERT INTO theRenewalLoanDetailsN 
                VALUES (
                    @accountNumber, @loansOfficer, @balanceDueN, @originalRate, 
                    @trnDate, @initialDisDate, @loanTenure, periodSet, 
                    @theBuzId, @numberOfRenewals, @previousId, theRateTypeUsed, 
                    thePeriodDecoded(TRIM(SUBSTRING_INDEX(@originalTenure, ' ', -1))), 
                    TRIM(SUBSTRING_INDEX(@originalTenure, ' ', -1))
...
-- DELETE pmms_loans.new_loan_appstore1,loandisburserepaystatement from pmms_loans.new_loan_appstore1 INNER JOIN loandisburserepaystatement ON new_loan_appstore1.trn_id=loandisburserepaystatement.loanTrnId WHERE new_loan_appstore1.loan_cycle_status='Completed' AND new_loan_appstore1.trn_date<='2023-08-31';
...
SELECT loanTrnId  INTO loanTrnIdL FROM   loandisburserepaystatement WHERE BatchCode=batchNumber;
...
-- DELETE pmms_loans.new_loan_appstore1,loandisburserepaystatement from pmms_loans.new_loan_appstore1 INNER JOIN loandisburserepaystatement ON new_loan_appstore1.trn_id=loandisburserepaystatement.loanTrnId WHERE new_loan_appstore1.loan_cycle_status='Completed' AND new_loan_appstore1.trn_date<='2024-03-31';
