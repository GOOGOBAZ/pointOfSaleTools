...
BEGIN
    ...
    read_loop: LOOP
        ...
        -- Perform the necessary processing for each loanOfficerId
        INSERT INTO smsSummuryOfficer (officerId, noOfCustomer, paidC, percentPaid, portfolioAmount, amountPaid, newCustomers)
        SELECT 
            loanOfficerId AS officerId,
            CONCAT('noOfCustomer:', IFNULL(NumberOfCustomers(loanOfficerId), '0')) AS noOfCustomer,
            CONCAT('paidC:', IFNULL(PaidCustomers(loanOfficerId), '0')) AS paidC,
            CONCAT('percentPaid:', IFNULL(CONCAT(PercentPaidCustomers(loanOfficerId), '%'), '0%')) AS percentPaid,
            CONCAT('portfolioAmount:', IFNULL(PortfolioAmount(loanOfficerId), '0')) AS portfolioAmount,
            CONCAT('amountPaid:', IFNULL(AmountPaid(loanOfficerId), '0')) AS amountPaid,
            CONCAT('newCustomers:', IFNULL(newCustomers(loanOfficerId), '0')) AS newCustomers;
